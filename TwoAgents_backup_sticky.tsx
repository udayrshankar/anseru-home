...
export default function TwoAgents() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const newAgent = progress < 0.5 ? 'sud' : 'kg';
  ...
  const scrollToAgent = (agent: 'sud' | 'kg') => {
    ...
    const targetScrollY = absoluteTop + (agent === 'sud' ? 0 : scrollableDistance);
  ...
  return (
    <div ref={containerRef} className="w-full bg-white relative h-[200vh]">
      <div className="sticky top-0 h-screen w-full flex flex-col justify-center py-16 md:py-24 overflow-hidden">
        <div className="max-w-[1200px] mx-auto px-6 w-full flex flex-col h-full justify-center">

          {/* ── Header ──────────────────────────────────────────── */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 md:mb-12 gap-6 flex-shrink-0">
          ...
          <div className="border border-gray-300 rounded-2xl overflow-hidden flex-shrink-0">
            <div
              className="flex h-full w-[200%] transition-transform duration-75 ease-out will-change-transform"
